window.bankOrderCall = async function(name, data, user) {
  if (!['createBankOrder', 'approveBankOrder', 'downloadBankPurchase', 'manageBankPayout', 'manageBankDispute', 'reportBankDispute'].includes(name) || !user) throw Error('Sign in to continue.');
